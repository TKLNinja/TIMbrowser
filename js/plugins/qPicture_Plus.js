...
//=============================================================================
/*:
 * @plugindesc <QPicture_V3>
 * Adds additional features to Pictures
 * @version 1.1.3
 * @author Quxios + ShdwRCK | Version 1.1.3
 ...
 *  - for setVarX : number you want to set VarX to
 *  - for addVarX : number you want to add to varX (positive or negative)
 *
 * ----------------------------------------------------------------------------
 * **Click Through Alpha**
 * ----------------------------------------------------------------------------
...
(function() {
  ...
  Game_Interpreter.prototype.qPictureCommand = function(args) {
    ...
    if (/^on/.test(cmd)) {
      ...
      var incVarId = /^incVar(\d+)/.exec(args[0]);
      var decVarId = /^decVar(\d+)/.exec(args[0]);
      var setVarId = /^setVar(\d+)/.exec(args[0]);
      var addVarId = /^addVar(\d+)/.exec(args[0]);
      if (switchId) {
        ...
        pic.setMouseHandler(cmd, function(id) {
          var varia = $gameVariables.value(id) + 1;
          $gameVariables.setValue(id, varia);
          console.log(varia);
        }.bind(pic, Number(incVarId[1])));
      } else if (decVarId) {
        pic.setMouseHandler(cmd, function(id) {
          var varia = $gameVariables.value(id) - 1;
          $gameVariables.setValue(id, varia);
          console.log(varia);
        }.bind(pic, Number(decVarId[1])));
      } else if (setVarId) {
        pic.setMouseHandler(cmd, function(id) {
          var varia = Number(args[1]);
          $gameVariables.setValue(id, varia);
        }.bind(pic, Number(setVarId[1], args[1].toLowerCase())));

      } else if (addVarId) {
        pic.setMouseHandler(cmd, function(id) {
          var varia = $gameVariables.value(id) + Number(args[1]);
          $gameVariables.setValue(id, varia);
        }.bind(pic, Number(addVarId[1], args[1].toLowerCase())));
      }
      return;
    }
    if (cmd === 'ease') {
